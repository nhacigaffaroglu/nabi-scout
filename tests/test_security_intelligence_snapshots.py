...
class SnapshotIdempotencyTests(unittest.TestCase):
    ...
    def test_consumers_read_snapshot_and_keep_live_evaluate(self) -> None:
        ...
        self.assertIn("publish_canonical_security_intelligence", page)
        self.assertIn("SI snapshot'ını yayımla", page)
        self.assertIn("bist_readiness_applies", page)
        self.assertIn("RESOLUTION_RESOLVED", page)
        self.assertNotIn("identity_ok=security_resolution is not None", page)
        self.assertNotIn("save_security_intelligence_snapshot(", page)
