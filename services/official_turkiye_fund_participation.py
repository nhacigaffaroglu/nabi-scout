...
MIXED_ALLOWED = frozenset(
    LIQUIDITY_ALLOWED
    | EQUITY_ALLOWED
    | SUKUK_ALLOWED
    | PRECIOUS_METALS_ALLOWED
)
REAL_ESTATE_ALLOWED = frozenset(
    {ASSET_GROUP_OTHER, ASSET_GROUP_CASH, ASSET_GROUP_PARTICIPATION_ACCOUNT, ASSET_GROUP_REPO, ASSET_GROUP_FUND}
)
...
REAL_ESTATE_PROFILES = frozenset(
    {PROFILE_REAL_ESTATE_PARTICIPATION, PROFILE_REAL_ESTATE_PARTICIPATION_FUND}
)
MIXED_PROFILES = frozenset(
    {PROFILE_MIXED_MULTI_ASSET_PARTICIPATION, PROFILE_MIXED_MULTI_ASSET_PARTICIPATION_FUND}
)
...
def _allowed_groups(official_profile: Optional[str]) -> frozenset[str]:
    ...
    if official_profile in MIXED_PROFILES:
        return MIXED_ALLOWED
    if official_profile in REAL_ESTATE_PROFILES:
        return REAL_ESTATE_ALLOWED
    return frozenset()
